...
namespace MalignEngine
{
    public class FontSystem : BaseSystem, IFontStashRenderer2
    {
        ...
        {
            RenderingSystem.DrawQuad((Texture2D)texture, topRight, bottomRight, bottomLeft, topLeft);
        }
        ...
        public void DrawFont(Font font, int fontSize, string text, Vector2 position, Color color, float rotation, Vector2 origin = default, Vector2 scale = default)
        {
            ...
            if (scale.Y < 0f) { RenderingSystem.FlipY = false; }
            ...
            if (scale.Y < 0f) { RenderingSystem.FlipY = true; }
        }
        ...
        {
            RenderingSystem.DrawQuad((Texture2D)texture,
