...
public class ImGuiSystem : BaseSystem
{
        ...
        : base(loggerService, scheduleManager)
    {
        ...
        _glRenderAPI.Submit(() =>
        {
            _imGuiController = new ImGuiController(_glRenderAPI.InternalAPI, windowContext.GetWindow(), inputContext.InputContext, () =>
            {
    ...
    public void Image(ITextureResource texture, Vector2 size, Vector2 uv0, Vector2 uv1)
    {
        ImGui.Image((IntPtr)((GLTextureResource)texture).GetHandle(), size, uv0, uv1);
    }
    ...
    {
        return ImGui.ImageButton(id, (IntPtr)((GLTextureResource)texture).GetHandle(), size);
    }

    public bool ImageButton(string id, ITextureResource texture, Vector2 size, Vector2 uv1, Vector2 uv2)
    {
        return ImGui.ImageButton(id, (IntPtr)((GLTextureResource)texture).GetHandle(), size, uv1, uv2);
    }
}