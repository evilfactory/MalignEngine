...
namespace MalignEngine
{
    ...
    internal static class MsgWriter
    {
        ...
        internal static void WriteBoolean(ref byte[] buf, ref int bitPos, ref int bitLength, bool val)
        {
            ...
            {
                throw new Exception($"Boolean written incorrectly! {testVal}, {val}; {resetPos}, {bitPos}");
            }
#endif
        }
    ...
    public sealed class ReadOnlyMessage : IReadMessage
    {
        private int seekPos;
        private int lengthBits;
        ...
        public ReadOnlyMessage(byte[] inBuf, bool isCompressed, int startPos, int byteLength)
        {
            if (isCompressed)
            {
